...
function TestDetails() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const response = await fetch(`http://52.66.237.233:3000/api/testDrive/get/${id}`, {
          method: "GET",
          headers: {
  ...
  useEffect(() => {
    async function fetchCarData(carId) {
      ...
      const url = `http://52.66.237.233:3000/api/cars/cars/${carIdString}`;
  ...
  async function handleCancel() {
    try {
      const response = await fetch(`http://52.66.237.233:3000/api/testDrive/${id}/cancel`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
  ...
  async function confirmDelete() {
    try {
      const response = await fetch(`http://52.66.237.233:3000/api/testDrive/delete/${id}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
  ...
  async function handleUpdate() {
    try {
      const response = await fetch(`http://52.66.237.233:3000/api/testDrive/update/${id}`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
